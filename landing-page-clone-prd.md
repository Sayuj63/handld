...
### For: [Your Brand — assuming Handld unless told otherwise]

> **Assumption flag:** The reference (umanodesign.studio) sells "unlimited design requests, fixed monthly price, pause/cancel anytime." That value-prop maps almost 1:1 onto Handld ("unlimited change requests, fixed price, no more WhatsApp chaos"). I've written the copy-direction notes assuming this is Handld's marketing site. If it's for something else, say so and I'll redirect the copy — the structure/animation spec below doesn't change either way.
...
- Headline: two-line, huge, display font, black text on orange — reference copy: "Pause hiring, Start designing." → **your copy direction:** "Stop chasing developers, Start shipping." or similar (swap "hiring a designer" language for "waiting on dev changes")
- Subheadline: one line, medium weight, below headline
- Below that: an iPhone mockup (hand holding phone, dark silhouette fingers), phone screen shows a notification-style card: small brand icon + "Get the job done" eyebrow label + bold headline "Your design has been delivered" + a blurred content preview below
...
**Video prompt — hero phone mockup:** see §7 (flagged for hybrid build, not pure AI video — read why before generating).
...
- Your version: swap for your own client logos (7–8 Shopify brands) — get written permission before using client logos publicly if you don't already have it in your contracts
...
- **Your copy direction:** "Chasing developers for every small fix takes days. Submitting it on [Brand] takes seconds. Unlimited requests. Fixed monthly price. No commitment."
...
| Card | Visual | Heading | Copy |
|---|---|---|---|
| A | 3D character illustration, waving, orange outfit | "Your designer, from day one" | "A senior product designer fully dedicated to your company. Working like a founding designer, without the hiring process." |
| B | Dark UI mockup card ("Design the onboarding" task) | "Unlimited design requests" | "Submit as many tasks as you need. No per-task billing, no cap, no waiting list. Just continuous output." |
...
**Your copy direction for Cards A/B:** "Your Shopify dev, from day one" / "Unlimited change requests" — same structure, swap "designer" → "developer" and "design tasks" → "change requests."

### 4.5 Feature Pair — Workflow + Pricing Model
- Confirmed in Image 6, shown with "Client stories" highlighted as the active nav item (State B, not State C)
- Left card: integration icons (Trello, a starburst icon, an arrow/flow icon, a stylized "S" badge, a heart/gradient icon, Slack) with heading ending "...orkflow" (cut off — likely "Seamless workflow") and copy "Linear. We plug into [your tools], work like part of your team."
- Right card: 3D cloud + orange geometric shapes illustration, heading "Flexible and predictable," copy "One flat subscription. No surprises, no contracts. Pause or cancel anytime. Scale design up or down [as needed]."
...
**Your copy direction:** integration card → swap Trello/Slack icons for whatever you'll realistically integrate with (Shopify, email, Slack are the obvious ones for you). Flexible pricing card copy translates directly — "fixed monthly price, pause or cancel anytime" already matches your Handld pricing model almost verbatim.
...
- Headline: "Simple and transparent pricing." — large display font, white text
- Two pricing cards side by side: **Subscription** (from €260/day) and **Mission** (from €850/day)
...
**Your pricing copy direction:** "Subscription" → "Monthly plan," "Mission" → could map to a one-off/priority-project tier if you offer that, or drop this tier entirely if Handld is subscription-only.

### 4.8 FAQ Accordion
- Simple accordion, `+` icon that likely rotates to `×` on expand (standard pattern, 🟡 inferred)
- Confirmed questions from screenshots: "What can you design?" / "How fast can you start?" — plus a third above them cut off in Image 9
- **Your copy direction:** "What kind of changes can I submit?" / "How fast do requests get picked up?" / "Can I cancel anytime?"

### 4.9 Sticky Reveal Footer (your ask #8)
- Two-column block: left = solid brand-orange card with logo + tagline ("Senior product design, on demand.") stacked on white/cream bg; right = footer nav columns (Links / Company) + copyright line + secondary "Book a call" button
- Below that: **a giant, low-contrast, edge-to-edge email address** ("hi@umanodesign.studio") rendered in huge type against the page background — barely-there contrast, clearly decorative rather than a functional link at that size
...
- **Your copy direction:** giant text = "hi@[yourdomain].com" or your actual brand wordmark
...
1. **Confirm the brand/copy direction** — I wrote this assuming it's Handld's marketing site. If not, tell me the actual product and I'll rewrite every copy suggestion in §4.
...
3. **Client logos** — do you have permission to display your 7–8 client logos publicly on the landing page, or do you need a generic "trusted by growing Shopify brands" line instead?
